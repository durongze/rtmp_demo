...
LIB_OPENSSL=-lssl -lcrypto $(LIBZ) $(LIB_GNUTLS)
...
PROGS=rtmpdump rtmpgw rtmpsrv rtmpsuck rtmppublish
...
install:	$(PROGS)
	...
	cp rtmpgw$(EXT) rtmpsrv$(EXT) rtmpsuck$(EXT) rtmppublish$(EXT) $(SBINDIR)
	cp rtmpdump.1 $(MANDIR)/man1
	cp rtmpgw.8 $(MANDIR)/man8
	@cd librtmp; $(MAKE) install

clean:
	rm -f *.o rtmpdump$(EXT) rtmpgw$(EXT) rtmpsrv$(EXT) rtmpsuck$(EXT) rtmppublish$(EXT)
...
rtmppublish: rtmppublish.o thread.o
	$(CC) $(LDFLAGS) -o $@$(EXT) $@.o thread.o $(SLIBS)
...
rtmppublish.o: rtmppublish.c $(INCRTMP) Makefile
